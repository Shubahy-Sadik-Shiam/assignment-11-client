...
const AddBook = () => {
    ...
    const handleAddBook = e=>{
        ...
        axios.post("http://localhost:5000/allBooks",initialData)
        .then((response)=>{
      ...
      <form onSubmit={handleAddBook} className="my-4">
        ...
        <div className="md:flex gap-3">
          ...
          <div>
            ...
            <select name="category" required defaultValue="Select a category" className="select select-bordered w-60">
              <option disabled>
                Select a category
              </option>
              <option>Novel</option>
        ...
        {/* submit button */}
        <input className="btn btn-block my-3" type="submit" value="Add Book" />
        </div>
      </form>
    </div>
  );
